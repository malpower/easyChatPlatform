    ...
    //模拟数据
     ...
     $.ajax({
			...
			$.ajax({
		 	  	...
		 	  	data:JSON.stringify({
		 	  		category: 'Samples',   //积分
					conditions: {"liked.user":res.list[0]._id},
						...
						for(var a = 0;a<resss.list.length;a++){
							$('#mycollectList').append(
								 '<li><a href="detailes.html?caseid='+resss.list[a]._id+'">'+
           						 '<div class="fl ui-img" style="background: url(/getImage?id='+resss.list[a]._id+')no-repeat center;background-size: 100% 100%;"></div>'+
